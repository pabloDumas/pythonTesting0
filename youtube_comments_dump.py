...
# --------- Config ----------
API_KEY = os.getenv("YOUTUBE_API_KEY")  # <-- same env var as your other script
...
def fetch_replies(parent_id: str, text_format: str = "plain"):
    """
...
def fetch_all_comments(video_id: str, text_format: str = "plain"):
    """
    ...
    while True:
        ...
        if not items and not data.get("nextPageToken"):
            # No comments or comments may be disabled
            break
...
def save_comments_to_file(comments, video_id: str, out_dir: str | None = None) -> str:
    """
    ...
    fname = f"youtube_comments_{video_id}_{time.strftime('%Y%m%d-%H%M%S')}.txt"
...
def parse_args():
    ...
    parser.add_argument(
        "--out-dir",
        default=None,
        help="Directory to save the .txt file (default: system temp dir)",
    )
...
def main():
    ...
    out_path = save_comments_to_file(comments, video_id, args.out_dir)
